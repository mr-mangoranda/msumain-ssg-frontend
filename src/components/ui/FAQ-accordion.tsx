          ...
          {FaqAccordionData.map((accordion, i) => (
            <AccordionItem
              ...
              className={`rounded-4xl overflow-hidden mb-5 ${
                isPrivacyPolicy
                  ? "bg-white border-2 border-mainmaroon"
                  : "bg-[#EDEDED]"
              }`}
            >
              <AccordionTrigger
                className={
                  ...
                  "[&>svg]:bg-mainmaroon [&>svg]:text-white [&>svg]:rounded-full [&>svg]:p-2 [&>svg]:w-9 [&>svg]:h-9 [&>svg]:flex [&>svg]:items-center [&>svg]:justify-center"
                }
              >
                <span 
                  className="text-left"
